...
sep_old <- 
  ...
  filter(u_grad > 10) %>%
  group_by(title) %>%
  summarize(u_grad = sum(u_grad), id = id, title = title, name = name, department = department) %>%
  distinct()
sep_old
...
* I would look at the classes, and I would merge them
* I would tell her that she can't get any of the data, its mine
